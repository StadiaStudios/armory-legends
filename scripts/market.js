...
const MarketSystem = {
    ...
    render(inventory, repository, upgrades = {}) {
        ...
        // Filter inventory to exclude limited items so they don't show up in the sell list
        const sellableInventory = inventory.filter(w => {
            const lookupId = w.modelId || 'pistol_9mm';
            const weaponDef = repository[lookupId];
            return !(weaponDef && weaponDef.isLimited);
        });

        if (sellableInventory.length === 0) {
            marketContainer.innerHTML = `<div class="col-span-3 text-center py-20 text-zinc-600 font-bold uppercase tracking-widest">WIFI SIGNAL WEAK: No Sellable Inventory</div>`;
            return;
        }
        ...
        marketContainer.innerHTML = this.buyers.map(b => `
            <div class="bg-zinc-900 p-6 rounded-lg neon-border border-l-4" style="border-color: ${b.color}">
                ...
                <div class="space-y-4 border-t border-zinc-800 pt-4">
                    ...
                    ${sellableInventory.slice(0, 3).map(w => {
                        ...
                        // Items are already filtered, so we don't need to check isLimited here
                        ...
                        let buttonText = 'SELL';
                        if (isOnCooldown) buttonText = `WAIT ${remainingSec}s`;
